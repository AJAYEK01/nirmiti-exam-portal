...
async function main() {
  ...
  const webDevQuestions = [
    {
      ...
      correctAnswer: 1,
      explanation: "The event loop continuously checks if the call stack is empty. When empty, it dequeues tasks from the microtask queue (Promises) and macrotask queue (setTimeout) onto the call stack."
    },
    {
      orderIndex: 2,
  ...
  const aptitudeExam = await prisma.exam.create({
    data: {
      ...
      description: "Evaluate your numerical agility, pattern recognition, and logical deduction abilities. 5 questions with +2 marks per correct answer and -0.5 penalty for wrong answers.",
      category: "Aptitude",
      durationMinutes: 15,
      totalMarks: 10,
      passingMarks: 6,
  ...
  console.log("Database seeded successfully with users and exams!");
}
